...
describe 'Session', ->
  before (done) ->
    ...
    require [
      ...
      require [
        ...
        'domain_extractor'
        'helpers/uuid_helper'
      ], (Session, PluginsManager, Promise, Biskoto, Settings, AnalyticsUrl, DomainExtractor, UUID) =>
        @session = Session
        ...
        @uuid_stub = sinon.stub(UUID, 'generate').returns(@transaction_id)
  ...
  describe '.transaction_id', ->
    it 'is generated by the UUID helper', ->
      @instance = new @session(@plugins_manager)

      expect(@instance.transaction_id).to.eq(@transaction_id)
